  ...
  <div>
    ...
    <section class="mainwrap">
      <div class="header">
        ...
        <div>
          <el-button style="margin-right: 20px" @click="handleBatchAdd">批量发放</el-button>
          <el-checkbox v-model="checked" @change="handleChecked">显示欠教材学生</el-checkbox>
        </div>
      </div>
      ...
      <div class="userTable">
        <el-table ref="multipleTable" :data="listData" tooltip-effect="light" v-loading="listLoading" element-loading-text="loading" element-loading-spinner="el-icon-loading" element-loading-background="#fff" stripe style="width: 100%" class="min_table" :header-cell-style="{ 'text-align': 'center' }" :cell-style="{ 'text-align': 'center' }" @selection-change="handleSeletChange">
          ...
          <el-table-column prop="project_name" label="所属项目" min-width="160" show-overflow-tooltip></el-table-column>
          <!-- <el-table-column
            prop="classroom_id"
            label="所属班级"
            min-width="100"
          ...
          ></el-table-column> -->
          <el-table-column prop="total_books" label="应发教材" min-width="80" show-overflow-tooltip></el-table-column>
          <el-table-column prop="not_issued" label="欠发教材" min-width="100" show-overflow-tooltip>
            <template slot-scope="{ row }">
              <span>{{ row.not_issued ? row.not_issued : "已发放完成" }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="update_time" label="最后更新时间" min-width="160" show-overflow-tooltip>
          </el-table-column>

          <el-table-column label="操作" fixed="right" min-width="130">
            <template slot-scope="{ row }">
              <div class="operation_btn">
                <el-button type="text" @click="handleAdd(row.id, row.project_id)">教材发放</el-button>
                <el-button type="text" @click="toMaterialJournal(row.id)">日志</el-button>
              </div>
            </template>
          </el-table-column>
        </el-table>
      ...
      <!-- 发放教材 -->
      <GrantTeachMaterials v-model="dialogVisible" :ids="checkedIds" :projectId="projectId" @on-success="dispenseList" />
    </section>
  </div>
</template>
