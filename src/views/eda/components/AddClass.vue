<template>
  <!--添加班级弹框-->
  <el-dialog :title="title" :visible.sync="visible" width="700px" class="add-class" @open="handleOpen" :close-on-click-modal="false" @closed="resetForm('formData')">
    <el-form v-loading="detailLoading" label-width="100px" :model="formData" :rules="rules" ref="formData">
      <el-row>
        <el-col :lg="12" :xs="12" :sm="12" :xl="12">
          <el-form-item label="所属分类" prop="category_id">
            <el-cascader style="width: 100%" v-model="formData.category_id" placeholder="请选择所属分类" :options="typeOptions" filterable clearable :props="{ checkStrictly: true }" @change="handleTypeChange">
            </el-cascader>
          </el-form-item>
        </el-col>
        <el-col :lg="12" :xs="12" :sm="12" :xl="12">
          <el-form-item label="所属项目" prop="project_id">
            <el-select style="width: 100%" :disabled="!formData.category_id" v-model="formData.project_id" placeholder="请选择所属项目" @change="handleProjectChange">
              <el-option v-for="item in projectOptions" :key="item.project_id" :label="item.project_name" :value="item.project_id">
              </el-option>
            </el-select>
          </el-form-item>
        </el-col>
      ...
      <el-row>
        <el-col :lg="12" :xs="12" :sm="12" :xl="12">
          <el-form-item label="所属课程" prop="course_id">
            <el-select style="width: 100%" :disabled="!formData.project_id" v-model="formData.course_id" placeholder="请选择所属课程">
              <el-option v-for="item in courseOptions" :key="item.course_id" :label="item.course_name" :value="item.course_id">
              </el-option>
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :lg="12" :xs="12" :sm="12" :xl="12">
          <el-form-item label="班级名称" prop="classroom_name">
            <el-input placeholder="请输入班级名称" v-model="formData.classroom_name"></el-input>
          </el-form-item>
        </el-col>
      </el-row>
      ...
      <el-row>
        <el-col :lg="12" :xs="12" :sm="12" :xl="12">
          <el-form-item label="班级封面" prop="class_icon">
            <div class="class-cover-upload">
              <el-upload class="upload-item" name="image" :headers="headers" :action="uploadImageUrl" :show-file-list="false" :on-error="handleUploadError" :on-success="handleUploadSuccess" :before-upload="beforeUpload">
                <div v-if="formData.class_icon" class="imgs">
                  <img :src="formData.class_icon" />
                  <i class="del el-icon-close" @click.stop="hanldeDelete"></i>
                </div>
                <i v-if="!uploadLoading && !formData.class_icon" class="el-icon-plus upload-item-icon"></i>
        ...
        <el-col :lg="12" :xs="12" :sm="12" :xl="12">
          <el-form-item label="班主任" prop="master_teacher_id">
            <el-select style="width: 100%" v-model="formData.master_teacher_id" placeholder="请选择">
              <el-option v-for="item in staffOptions" :key="item.staff_id" :label="item.staff_name" :value="item.staff_id">
              </el-option>
            </el-select>
          </el-form-item>
        </el-col>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        classroom_name: [{ required: true, message: '请输入班级名称', trigger: 'blur' }],
        ...
        master_teacher_id: [{ required: true, message: '请选择', trigger: 'change' }],
        class_icon: [{ required: true, message: '请选择图片', trigger: 'change' }],
      },
      addLoading: false,
      uploadLoading: false,
      detailLoading: false,
