  ...
  <section>
        ...
        <el-table-column label="操作" fixed="right" min-width="120">
          <template slot-scope="{ row }">
            <template v-if="!row.verify_status">
              <el-button
                v-if="row.pay_money * 1 >= 0"
                type="text"
                @click="openEntryDialog(row)"
                >入账</el-button
              >
              <el-button v-else type="text" @click="openEntryDialog(row)"
                >出账</el-button
              >
              <el-button type="text" @click="rejectConfirm(row.id, 2)"
                >驳回</el-button
              >
            </template>
            <el-button
    ...
    >
      <el-form :model="formData" :rules="rules" ref="formData">
        <el-form-item
          :label="`${checkedPayMoney >= 0 ? '入' : '出'}账时间`"
          prop="pay_date"
        >
          <el-date-picker
            class="input"
...
export default {
  ...
  methods: {
    ...
    openEntryDialog(row) {
      this.checkedPayMoney = row.pay_money * 1;
