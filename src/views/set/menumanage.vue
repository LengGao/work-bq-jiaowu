<template>
  <section class="mainwrap">
    ...
    <!--表格-->
    <div class="userTable">
      <el-table ref="multipleTable" :data="schoolData" row-key="id" style="width: 100%" :tree-props="{ children: 'children' }" class="min_table" :header-cell-style="{ 'text-align': 'center' }" :cell-style="{ 'text-align': 'center' }">
        <el-table-column label="菜单名称" show-overflow-tooltip min-width="120" prop="menu_name">
        </el-table-column>
        <el-table-column label="排序" min-width="60" show-overflow-tooltip>
          <template slot-scope="scope">
            <el-col :span="12" v-if="scope.row.menu_type == 1">
              <el-input style="width: 56px" v-model="scope.row.sort" placeholder size="small" @blur="scopes(scope.row.id, scope.row.sort)"></el-input>
            </el-col>
          </template>
        </el-table-column>
        <el-table-column prop="menu_type_name" label="类型" min-width="110" show-overflow-tooltip></el-table-column>
        ...
        <el-table-column prop="auth" label="接口地址" min-width="110" show-overflow-tooltip></el-table-column>
        <el-table-column prop="node" label="前端标识" min-width="110" show-overflow-tooltip></el-table-column>
        <el-table-column prop="index_category_name" label="启用状态" min-width="110" show-overflow-tooltip>
          <template slot-scope="scope">
            <!-- <el-switch :active-value="1" :inactive-value="0"></el-switch> -->
            <el-switch v-model="scope.row.menu_status" :active-value="1" :inactive-value="0" @change="switchStatus(scope.row)"></el-switch>
          </template>
        </el-table-column>
        <el-table-column prop="index_category_name" label="是否在菜单显示" min-width="110" show-overflow-tooltip>
          <template slot-scope="{ row }">
            <!-- <el-switch :active-value="1" :inactive-value="0"></el-switch> -->
            <el-switch :active-value="1" :inactive-value="0" v-model="row.show_at_list" @change="updateShowStatus(row)"></el-switch>
          </template>
        </el-table-column>
        <el-table-column label="操作" fixed="right" min-width="200">
          <template slot-scope="scope">
            <div style="display: flex; justify-content: center">
              ...
              <el-button type="text" style="padding-left: 40px" @click="delbtn(scope.row)">删除</el-button>
            </div>
          </template>
        </el-table-column>
      </el-table>
    ...
    <!--弹框-->
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" :close-on-click-modal="false" width="576px">
      <el-form label-width="100px">
        <el-form-item label="上级菜单">
          <el-cascader v-model="parent_id_arr" :options="ThumbData" :props="optionProps" clearable @change="checkDepart"></el-cascader>
        </el-form-item>
        <el-form-item :label="ruleForm.menu_type == 1 ? '菜单名称' : '功能名称'">
          <el-input :placeholder="
              ruleForm.menu_type == 1 ? '请输入菜单名称' : '请输入功能名称'
            " v-model="ruleForm.menu_name" class="input-width"></el-input>
        </el-form-item>
        <el-form-item label="图标" v-if="ruleForm.menu_type == 1">
          <div class="form-select-icon">
            <i v-if="ruleForm.icon" style="font-size: 40px; margin-right: 10px" class="iconfont" :class="ruleForm.icon"></i>
            <el-button @click="iconDialog = true">选择图标</el-button>
          </div>
        </el-form-item>
        <el-form-item label="接口地址">
          <el-input placeholder="请输入接口地址" v-model="ruleForm.auth" class="input-width"></el-input>
        </el-form-item>
        <el-form-item label="前端标识">
          <el-input placeholder="请输入前端标识" v-model="ruleForm.node" class="input-width"></el-input>
        </el-form-item>
        ...
        <el-form-item label="菜单排序" prop="sort" v-if="ruleForm.menu_type == 1">
          <el-input placeholder="请输入菜单排序" v-model="ruleForm.sort" class="input-width" type="number"></el-input>
    ...
    <!-- <imgDialog
      ...
      @clearUrl="clearUrl"
    ></imgDialog> -->
    <el-dialog title="选择图标" :visible.sync="iconDialog" width="400px" top="30px" class="grant-teach-materials">
      <ul class="icon-list">
        <li v-for="(icon, index) in iconList" :key="index" :class="icon === ruleForm.icon ? 'active' : ''" @click="handleIconChange(icon)">
