<template>
  <div>
    <section class="mainwrap">
      <div>
        <el-form :model="formInline" label-width="80px" class="demo-form-inline" label-position="left">
          <el-form-item label="角色名称">
            <el-input v-model="formInline.user" placeholder="校长" maxlength="30" style="width: 420px"></el-input>
          </el-form-item>
          <el-form-item label="备注">
            <el-input type="textarea" v-model="formInline.remark" placeholder="请输入" maxlength="100" style="width: 420px"></el-input>
          </el-form-item>
        </el-form>
        <span class="authority">功能权限</span>
        <div class="tree-container" v-loading="treeLoading">
          <el-tree ref="trees" v-for="(item, index) in roleData" :key="index" :data="item" show-checkbox :props="{
              label: 'title',
            }" :expand-on-click-node="false" check-on-click-node default-expand-all node-key="node_id" class="configure">
          </el-tree>
        </div>
        <div class="tree-footer">
          <el-button @click="handleCancel">取 消</el-button>
          <el-button type="primary" :loading="btnLoading" @click="handleSubmit">提 交</el-button>
        </div>
      </div>
    </section>
  </div>
