  ...
  data() {
    return {
      ...
      rules: {
        ...
        cate_id: [
          { required: true, message: "请选择科分类", trigger: "change" },
        ],
        plan_name: [
  ...
  methods: {
    ...
    async planInfo() {
      ...
      if (res.code === 0) {
        ...
        this.formData.date1 = [
          res.data.exam_start_time,
          res.data.exam_end_time,
        ];
        this.formData.date2 = [
          res.data.enroll_start_time,
          res.data.enroll_end_time,
        ];
