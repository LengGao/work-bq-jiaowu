<template>
  <!--添加编辑课时弹窗-->
  <el-dialog :title="title" :visible.sync="visible" width="450px" @open="handleOpen" :close-on-click-modal="false" @closed="resetForm('formData')">
    <el-form label-width="100px" :model="formData" :rules="rules" ref="formData" v-loading="detaiLoading">
      <el-form-item label="资料名称" prop="file_name">
        <el-input v-model="formData.file_name" placeholder="请输入资料名称" maxlength="20" />
      </el-form-item>
      <el-form-item label="上传资料">
        <el-upload ref="upload" :data="formData" :headers="headers" :action="uploadUrl" :on-error="handleUploadError" :on-success="handleUploadSuccess" :auto-upload="false" :before-upload="beforeUpload">
...
<script>
import { updateCourseMaterial, addCourseMaterial, getCourseMaterialDetail } from '@/api/sou'
