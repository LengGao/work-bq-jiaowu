<template>
  <div>
    ...
    <section class="mainwrap">
      ...
      <div class="userTable">
        <el-table ref="multipleTable" :data="listData" v-loading="listLoading" element-loading-text="loading" element-loading-spinner="el-icon-loading" element-loading-background="#fff" style="width: 100%" class="min_table" :header-cell-style="{ 'text-align': 'center' }" :cell-style="{ 'text-align': 'center' }">
          <el-table-column label="题库编号" show-overflow-tooltip min-width="70" prop="id">
          </el-table-column>
          <el-table-column prop="title" label="题库名称" min-width="180" show-overflow-tooltip></el-table-column>
          <el-table-column prop="category_name" label="所属分类" min-width="110" show-overflow-tooltip></el-table-column>
          <el-table-column prop="price" label="题库价格" min-width="110" show-overflow-tooltip>
            <template slot-scope="{ row }">
              <span>￥{{ row.price }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="book_price" label="是否上架" min-width="110" show-overflow-tooltip>
            <template slot-scope="{ row }">
              <el-switch @change="updateQuestionBankStatus(row)" v-model="row.status" :active-value="1" :inactive-value="0">
              </el-switch>
            </template>
          </el-table-column>
    ...
    <QuestionBankDialog v-model="dialogVisible" :title="dialogTitle" :typeOptions="typeOptions" :id="currentId" @on-success="getQuestionBankList" />
  </div>
</template>
