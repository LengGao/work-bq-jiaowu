<template>
  <div class="institutional-collection" v-loading="loading">
    <div class="student-order-title">
      <h3>
        {{ rebateData.institution_name }}-返点{{
      ...
      <div class="actions">
        <span v-if="rebateData.status == 0">
          <el-button type="primary" plain @click="dialogadopt()"
            >审批通过</el-button
          >
        ...
        <el-button type="primary" plain v-if="rebateData.status == 1"
          >已通过</el-button
        >
        <el-tag type="danger" plain v-if="rebateData.status == 2"
          >已驳回</el-tag
        >
      </div>
    </div>
        ...
        <div class="content">
          <span class="el-col-2">
            ...
            >
              待审批</el-tag
            >
            <el-tag v-if="rebateData.status == 1" class="tag-margin">
              已通过</el-tag
            >
            <el-tag
          ...
          <span class="el-col-6">
            <div
              class="list-img"
              v-if="rebateData.pay_proof_arr && rebateData.pay_proof_arr.length"
            >
              <img
                :src="item"
                alt=""
      ...
      >
        ...
        >
          <template slot-scope="{ row }">
            <el-button
              type="text"
              @click="toStudentOrderDetail(row.order_id)"
        ...
        >
          <template slot-scope="{ row }">
            ...
            <span
              ...
              >已通过</span
            >
            <span v-if="row.status == 2" class="approve-status">已驳回 </span>
          </template>
        </el-table-column>
        <el-table-column
          label="操作"
    ...
    >
      ...
      >
        ...
        <el-form-item label="关联订单数" prop="order_count">
          <el-input class="input" v-model="adoptData.order_count" disabled />
        </el-form-item>
        ...
        <el-form-item label="支付方式" prop="pay_type">
          <el-input class="input" v-model="adoptData.pay_type" disabled />
        </el-form-item>
        ...
        <el-form-item label="支付凭证" class="block">
          <ImgListUpload v-model="adoptData.pay_proof" />
        </el-form-item>
      </el-form>
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button
          type="primary"
          :loading="addLoading"
          @click="adoptsubmitForm('adoptData')"
          >确 定</el-button
        >
      </span>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    dialogadopt() {
      (this.id = this.$route.query.id), (this.dialogVisibleadopt = true);
      console.log(this.id);
      if (this.id) {
